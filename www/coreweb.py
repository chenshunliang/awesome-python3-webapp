...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        if name == 'request':
            ...
            continue
        if found and (
                            param.kind != inspect.Parameter.VAR_POSITIONAL and param.kind != inspect.Parameter.KEYWORD_ONLY and param.kind != inspect.Parameter.KEYWORD_ONLY):
            raise ValueError(
...
def add_route(app, fn):
    ...
    if path is None or method is None:
        raise ValueError('@get or @post nor defined in %s' % str(fn))
